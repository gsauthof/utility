...
// expands to parse_section_32() or parse_section_64()
static int CAT(parse_section_, PARGS_ELF_CLASS) (
    const Range *range, const char *filename, Landmarks *lm,
    const Range *section_range, uint32_t section_type,
    CAT3(uint, WIDTH, _t) section_virt_addr)
{
  CAT3(uint, WIDTH, _t) section_size =
    section_range->end - section_range->begin;
  switch (section_type) {
    case SHT_NOTE:
      debug("Reading NOTE section");
      return CAT(parse_notes_, WIDTH) (range, filename, lm, section_range);
    case SHT_PROGBITS:
      if (lm->execfn_addr > section_virt_addr
          && lm->execfn_addr < section_virt_addr + section_size) {
        debug("Found section that includes the execfn address");
        return CAT(find_vectors_, WIDTH) (filename, lm, section_range,
            section_virt_addr);
      } else {
        debug("Skipping PROGBITS section");
      }
      break;
    default: debug("Skipping other section");
  }
  return 0;
}
    ...
    const Range *range, const char *filename, Landmarks *lm)
{
  ...
  uint16_t first_section_count;
  memcpy(&first_section_count, b + offsetof(CAT3(Elf, WIDTH, _Ehdr), e_shnum),
      sizeof first_section_count);
  if (lm->need_to_swap)
    first_section_count = __builtin_bswap16(first_section_count);
  debug("%" PRIu16 " sections", first_section_count);
  if (!first_section_count) {
    fprintf(stderr, "File %s has no sections.\n", filename);
    return -1;
  }
  CAT3(uint, WIDTH, _t) section_header_off;
  memcpy(&section_header_off, b + offsetof(CAT3(Elf, WIDTH, _Ehdr), e_shoff),
      sizeof section_header_off);
  if (lm->need_to_swap)
    section_header_off = CAT(__builtin_bswap, WIDTH) (section_header_off);
  uint16_t section_header_size;
  memcpy(&section_header_size, b + offsetof(CAT3(Elf, WIDTH, _Ehdr),
        e_shentsize), sizeof section_header_size);
  if (lm->need_to_swap)
    section_header_size = __builtin_bswap16(section_header_size);
  if (section_header_off + section_header_size > e - b) {
    fprintf(stderr, "%s: section header table overflows\n", filename);
    return -1;
  }
  CAT3(uint, WIDTH, _t) section_count = first_section_count;
  // cf. the ELF standard, e.g. elf(5) and search forr SHN_LORESERVE
  if (section_count >= SHN_LORESERVE) {
    debug("file has more than 2**16-1 sections");
    const unsigned char *p = b + section_header_off;
    memcpy(&section_count, p + offsetof(CAT3(Elf, WIDTH, _Shdr), sh_size),
        sizeof section_count);
    if (lm->need_to_swap)
      section_count = CAT(__builtin_bswap, WIDTH) (section_count);
    debug("new section count: %" CAT(PRIu, WIDTH), section_count);
  }
  if (section_header_off + section_count * section_header_size > e - b) {
    fprintf(stderr, "%s: section header table overflows\n", filename);
    return -1;
  }
  CAT3(uint, WIDTH, _t) i = 0;
  for (const unsigned char *p = b + section_header_off
        + (section_count != first_section_count) * section_header_size;
      p < b + section_header_off + section_count * section_header_size;
      p += section_header_size, ++i) {
    debug("Reading %" CAT(PRIu, WIDTH) "th section", i+1);
    CAT3(uint, WIDTH, _t) section_off;
    memcpy(&section_off, p + offsetof(CAT3(Elf, WIDTH, _Shdr), sh_offset),
        sizeof section_off);
    if (lm->need_to_swap)
      section_off = CAT(__builtin_bswap, WIDTH) (section_off);
    CAT3(uint, WIDTH, _t) section_size;
    memcpy(&section_size, p + offsetof(CAT3(Elf, WIDTH, _Shdr), sh_size),
        sizeof section_size);
    if (lm->need_to_swap)
      section_size = CAT(__builtin_bswap, WIDTH) (section_size);
    if (section_off + section_size > e - b) {
      fprintf(stderr, "Section %" CAT(PRIu, WIDTH) " overflows %s.\n",
          i, filename);
      return -1;
    }
    uint32_t section_type;
    memcpy(&section_type, p + offsetof(CAT3(Elf, WIDTH, _Shdr), sh_type),
        sizeof section_type);
    if (lm->need_to_swap)
      section_type = __builtin_bswap32(section_type);

    CAT3(uint, WIDTH, _t) section_virt_addr;
    memcpy(&section_virt_addr, p + offsetof(CAT3(Elf, WIDTH, _Shdr), sh_addr),
        sizeof section_virt_addr);
    if (lm->need_to_swap)
      section_virt_addr = CAT(__builtin_bswap, WIDTH) (section_virt_addr);

    Range section_range = { b + section_off, b + section_off + section_size };
    int r = CAT(parse_section_, WIDTH) (range, filename, lm,
        &section_range, section_type, section_virt_addr);
    if (r)
      return r;
  }
  if (!lm->execfn_addr) {
